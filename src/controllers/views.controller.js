...
const realtimeproducts = async (req, res) => {
    ...
    try {
        ...
        res.render("realTimeProducts", { products: info.payload, user: res.locals.user, onlyAdmin: isAdmin, isPremium: premiumRole});
    } catch (error) {
...
const viewOneCart = async (req, res) => {
    ...
    try {
        ...
        res.render("cartView", { cart, user: res.locals.user, onlyAdmin: isAdmin, isPremium: premiumRole});
    } catch (error) {
        res.status(500).send({ error });
    }
}
...
const newProduct = async(req,res)=>{
    ...
    const premiumRole = res.locals.user && res.locals.user.role === 'premium';
    try {
        res.render("roleProducts",{user: res.locals.user, onlyAdmin: isAdmin, isPremium: premiumRole})
...
const myproducts = async (req, res) => {
    ...
    try {
        ...
        res.render("myProducts", { products, user: res.locals.user, onlyAdmin: isAdmin, isPremium: premiumRole });
    } catch (error) {
